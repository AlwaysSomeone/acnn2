...
model = pa.ACNN(N, embedding, DP, NP, K, NR, DC, KP)
...
def data_unpack(cat_data, target):
    ...
    be1 = Variable(torch.from_numpy(list_x[1]))
    be2 = Variable(torch.from_numpy(list_x[2]))
    bd1 = Variable(torch.from_numpy(list_x[3]))
    bd2 = Variable(torch.from_numpy(list_x[4]))
