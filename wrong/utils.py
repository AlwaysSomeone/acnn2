...
def load_embedding(config, word_dict):
  #word_dict是字典，句子所有单词按出现频率排序编号
  emb_file = config.embedding_file#embedding.txt
  emb_vocab = config.embedding_vocab#数据中所有的vocab
  ...
  f.close()
  #返回的embeddings是word_dict按照编号依次对应的向量表示
