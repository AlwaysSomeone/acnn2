...
def one_hot(indices, depth, on_value=1, off_value=0):
    ...
    if len(np_ids.shape) == 2:
        ...
        return Variable(torch.FloatTensor(added.astype(float)))
...
class ACNN(nn.Module):
    ...
    def window_cat(self, x_concat):
        ...
        '''
        t_px = torch.index_select(px, 1, Variable(torch.LongTensor(range(s[1]))))
        m_px = torch.index_select(px, 1, Variable(torch.LongTensor(range(1, s[1] + 1))))
        b_px = torch.index_select(px, 1, Variable(torch.LongTensor(range(2, s[1] + 2))))
